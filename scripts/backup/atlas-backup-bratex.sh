#!/bin/bash
# Atlas CMMS Backup and Restore Utility - VARIANTE SERVER BRATEX (websrv01)
#
# Differenze rispetto a scripts/backup/atlas-backup.sh (versione generica):
#   - BACKUP_DIR      -> /srv/data/backups/atlas_backups
#   - file .env       -> /srv/docker/atlas/.env  (percorso assoluto: lo script si puo'
#                        lanciare da qualsiasi directory, anche da cron)
#   - client mc       -> canale aistor
# Entrambi i percorsi si possono comunque sovrascrivere da ambiente, senza modificare
# il file:  BACKUP_DIR=/altro/path ./atlas-backup-bratex.sh backup
#
# Uso:  sudo ./atlas-backup-bratex.sh backup
#       sudo ./atlas-backup-bratex.sh restore atlas_backup_<TIMESTAMP>.tar.gz
...
# Percorsi del nostro server, sovrascrivibili da ambiente
BACKUP_DIR="${BACKUP_DIR:-/srv/data/backups/atlas_backups}"
ATLAS_ENV_FILE="${ATLAS_ENV_FILE:-/srv/docker/atlas/.env}"
...
#!/bin/sh
set -e
wget -q https://dl.min.io/aistor/mc/release/linux-amd64/mc -O /usr/bin/mc
chmod +x /usr/bin/mc
